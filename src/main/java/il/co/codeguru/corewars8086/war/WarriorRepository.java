...
public class WarriorRepository {
  ...
  public void readWarriorsFileFromPath(String path) throws IOException {
    ...
    if (warriorFiles == null) {
      if (options.headless) {
        System.err.printf("Error - survivors directory (\"%s\") not found%n", options.warriorsDir);
      } else {
        JOptionPane.showMessageDialog(null, "Error - survivors directory (\"" +
  ...
  private void fixFiles(File warriorsDirectory) {
    ...
    if (!warriorsDirectory.isDirectory()) {
      throw new RuntimeException(warriorsDirectory.getAbsolutePath() + " is not a directory");
    }
  ...
  public String getScores() {
    ...
    sorted.sort((o1, o2) -> Double.compare(o2.getGroupScore(), o1.getGroupScore()));
