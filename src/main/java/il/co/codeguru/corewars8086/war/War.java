...
public class War {
  ...
  public void loadWarriorGroups(WarriorGroup[] warriorGroups) throws Exception {
    m_currentWarrior = 0;
    ArrayList<WarriorGroup> groupsLeftToLoad = new ArrayList<>();
    Collections.addAll(groupsLeftToLoad, warriorGroups);
   ...
   * <p>
   * Since v5.0.0 (cgx2023), if the warrior is a zombie, the energy value will not decrease.
   *
   * @param warrior The warrior.
   * @param round   Current round number.
   */
  private void updateWarriorEnergy(Warrior warrior, int round) {
    if ((!warrior.isZombie()) && (round % DECELERATION_ROUNDS == 0)) {
  ...
  private void loadWarriorGroup(WarriorGroup warriorGroup) throws Exception {
    ...
    for (WarriorData warrior : warriors) {
      ...
      short energy = (short) (warrior.getType() == WarriorType.ZOMBIE ? options.zombieEnergy : 0);
